...
public class AddCollege extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			int result = cDAO.insertCollege(new College(college_name, bio, contact_email, address,fac_to_stu_ratio,placement_percent,min_percent,best_branch));
			RequestDispatcher rd = request.getRequestDispatcher("Home/Admin/AddCollegeDetails.html");
			rd.include(request, response);
			if(result>0) {						
				pw.println("<center>College Added Successfully...</center>");
			}else {			
				pw.println("<center>College not Added due to some Problem</center>");
